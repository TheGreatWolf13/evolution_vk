...
impl<V: VertexFormat> Pipeline<V> {
    ...
    // pub fn realloc_storage<R: IntoIterator<Item=WriteDescriptorSet>>(&mut self, len: usize, swapchain: &SwapChain, allocator: &Arc<StandardMemoryAllocator>, ds_allocator: &Arc<StandardDescriptorSetAllocator>, mut storage_binding_maker: impl FnMut(&Subbuffer<[V::SSBOType]>) -> R) {
    //     if len as DeviceSize > self.storage_buffers.get(swapchain).len() {
